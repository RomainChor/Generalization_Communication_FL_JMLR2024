...
class MLP(nn.Module):
    def __init__(self, dim_in, dim_hidden, dim_out, bias=True):
        ...
        self.layer_input = nn.Linear(dim_in, dim_hidden, bias)
        self.relu = nn.ReLU()
        self.dropout = nn.Dropout()
        self.layer_hidden = nn.Linear(dim_hidden, dim_out, bias)
...
class KernelSVM(nn.Module):
    def __init__(self, train_data_x, args):
        super().__init__()
        self._train_data_x = train_data_x.flatten(start_dim=1)
        ...
        if args.svm_kernel == 'linear':
            ...
            self._gamma = torch.nn.Parameter(torch.FloatTensor([args.gamma_init]),
                                             requires_grad=args.train_gamma)
        else:
            raise ValueError()
            
        self._w = torch.nn.Linear(in_features=self._num_c, out_features=1)
        self._flatten = torch.nn.Flatten(start_dim=1)
